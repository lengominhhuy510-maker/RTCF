...
rm_ss0  <- setNames(c(1.2,1.2,1.3,1.4,1.6), c("pack","pet","orange","mango","vitamin_c"))
rm_lot0 <- setNames(c(1L,1.3L,0.7L,1.5L,15L), c("pack","pet","orange","mango","vitamin_c"))

fg_ss0  <- setNames(c(1.4,2.0,1.3,1.7,1.2), unique(df_sku$sku))
fg_int0 <- setNames(c(10L,25,13,17,10,10), unique(df_sku$sku))  # 10 days all SKUs
frozen0 <- 3L
...
sales_customer_agreement0 <- tibble::tibble(
  ...
  promotional_pressure = "middle",
  promotion_horizon    = "short",
  service_level        = c(95.0,95.0,95.0),
  shelf_life           = c("80","80","80"),
  payment_term         = "3",
  order_deadline       = c("14pm","17pm","14pm"),
...
supplier_params_round0 <- tibble::tibble(
  ...
  quality = c("middle","poor","high","middle","high"),
  delivery_window = c("4hours","4hours","1day","1week","4hours"),
  delivery_reliability = c(98, 95.5, 96.5, 94.5, 95.0),
  trade_unit = c("pallet","pallet","ibc","ibc","drum"),
...
save_baseline_excel <- function(decisions, out_base, out_dir = "baseline_output") {
  if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
  
  # 1. Gom các bảng nghiệp vụ
  sheets <- list(
    sales_agreement      = decisions$sales$customer_product_level,
    assortment           = decisions$sales$assortment_cp,
    purchasing_agreement = decisions$purchasing$supplier_params,
    
    # 2. Tidy Supply Chain Knobs
    supply_chain_knobs = dplyr::bind_rows(
      dplyr::tibble(knob="fg_safety_stock_w", name=names(decisions$supply_chain$fg_safety_stock_w), 
                    value=as.numeric(decisions$supply_chain$fg_safety_stock_w)),
      dplyr::tibble(knob="rm_safety_stock_w", name=names(decisions$supply_chain$rm_safety_stock_w), 
                    value=as.numeric(decisions$supply_chain$rm_safety_stock_w)),
      dplyr::tibble(knob="rm_lot_size_w", name=names(decisions$supply_chain$rm_lot_size_w), 
                    value=as.numeric(decisions$supply_chain$rm_lot_size_w))
    ),
    
    # 3. Operations
    operations_knobs = dplyr::tibble(
      knob = names(decisions$operations),
      value = purrr::map_chr(decisions$operations, ~as.character(.x))
    ),
    
    # 4. Kết quả tài chính Baseline
    finance_summary = dplyr::tibble(
      metric = names(out_base$finance_round),
      value  = purrr::map_chr(out_base$finance_round, ~as.character(.x))
    )
  )
  
  path <- file.path(out_dir, "TFC_BASELINE_REPORT.xlsx")
  writexl::write_xlsx(sheets, path)
  
  message("✅ Đã lưu Baseline thành công tại: ", path)
}
save_baseline_excel(decisions = decisions_round, out_base = out_base)