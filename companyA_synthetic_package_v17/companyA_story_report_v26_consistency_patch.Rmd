...
```{r vsm-breakdown, echo=FALSE, message=FALSE, warning=FALSE}
p <- B$prod %>%
  mutate(
    cycle_min = as.numeric(difftime(step_end_time, step_start_time, units="mins"))
  ) %>%
...
# Top waiting step (for narrative text)
M$top_wait_step <- by_step$operation_step[1]
M$top_wait_avg  <- by_step$avg_wait[1]
M$top_wait_p95  <- by_step$p95_wait[1]

step_order <- by_step$operation_step

# Chart A: waiting only (so the story is readable)
hc_wait <- highchart() %>%
  hc_chart(type="bar") %>%
  hc_title(text="Where waiting happen by step (Avg waiting)") %>%
  hc_subtitle(text="Waiting is pure waste, we rank steps by avg waiting") %>%
  hc_xAxis(categories = step_order) %>%
...
# Chart B: cycle time only (processing load by step)
hc_cycle <- highchart() %>%
  hc_chart(type="bar") %>%
  hc_title(text="Where processing time sit (Avg cycle)") %>%
  hc_subtitle(text="Cycle is value-add time, but it also show heavy steps") %>%
  hc_xAxis(categories = by_step %>% arrange(desc(avg_cycle)) %>% pull(operation_step)) %>%
  hc_yAxis(title=list(text="Minutes")) %>%
  hc_add_series(
    name="Avg cycle",
    data=round(by_step %>% arrange(desc(avg_cycle)) %>% pull(avg_cycle), 1),
    color=COL_BEFORE
  ) %>%
  hc_tooltip(shared=TRUE) %>%
  hc_exporting(enabled = TRUE) %>%
...
# print both charts
htmltools::tagList(hc_wait, htmltools::br(), hc_cycle)
...
```{r quality-signal, echo=FALSE, message=FALSE, warning=FALSE}
# Use finished throughput (PACK) as denominator. Quality loss come from QC + rework loops.
...
rw_wk <- B$prod %>%
  ...
  mutate(week = wk(step_start_time)) %>%
  group_by(week) %>%
  summarise(rework = sum(rework_qty, na.rm=TRUE), .groups="drop")
...
```{r compare-metrics, echo=FALSE, message=FALSE, warning=FALSE}
compute_weekly <- function(x){
  ...
  # Throughput should be counted once at final step (PACK), not sum over all steps.
throughput_wk <- prod %>%
  filter(operation_step == "PACK") %>%
  group_by(week) %>%
  summarise(produced_qty = sum(produced_qty, na.rm=TRUE), .groups="drop")
...
full_join(throughput_wk, metrics_wk, by="week") %>%
  replace_na(list(produced_qty=0, rejected_qty=0, rework_qty=0)) %>%
...
```{r trust-weekly-waste-table, echo=FALSE, message=FALSE, warning=FALSE}
wk <- function(d) as.Date(floor_date(d, "week", week_start = 1))
...
prod_qc_wk <- B$prod %>%
  ...
  mutate(week = wk(step_end_time)) %>%
  group_by(week) %>%
  summarise(rejected_units = sum(rejected_qty, na.rm=TRUE), .groups="drop")

prod_rw_wk <- B$prod %>%
  filter(operation_step == "REWORK") %>%
  mutate(week = wk(step_end_time)) %>%
  group_by(week) %>%
  summarise(rework_units = sum(rework_qty, na.rm=TRUE), .groups="drop")

prod_wk <- prod_pack_wk %>%
  full_join(prod_qc_wk, by="week") %>%
  full_join(prod_rw_wk, by="week") %>%
  replace_na(list(produced_units=0, rejected_units=0, rework_units=0))
