...
```{r setup
COL_CAP  <- '#E74C3C'  # capacity reference line
, include=FALSE}
...
qwk <- B$prod %>%
  ...
  group_by(week) %>%
  summarise(
    produced = sum(produced_qty[operation_step == "PACK"], na.rm=TRUE),
    rejected = sum(rejected_qty[operation_step %in% c("QC_INLINE","QC_FINAL")], na.rm=TRUE),
    rework = {
      if ("rework_qty" %in% names(B$prod)) {
        sum(rework_qty[operation_step == "REWORK"], na.rm=TRUE) +
          sum(produced_qty[operation_step == "REWORK" & (is.na(rework_qty) | rework_qty == 0)], na.rm=TRUE)
      } else {
        sum(produced_qty[operation_step == "REWORK"], na.rm=TRUE)
      }
    },
    .groups="drop"
  ) %>%
  mutate(
    reject_rate = rejected / pmax(produced, 1),
    rework_rate = rework / pmax(produced, 1)
  )
...
# Throughput must be counted at the FINAL output step to avoid double counting across steps.
# Here we use PACK as the finished-goods output.
prod_wk <- B$prod %>%
  filter(operation_step == "PACK") %>%
...
dem_wk2 <- B$dem %>%
  ...
  group_by(week) %>%
  summarise(demand_qty = sum(demand_qty, na.rm=TRUE), .groups="drop")
...
highchart() %>%
  ...
  hc_add_series(td, "line", hcaes(x=week_ts, y=produced_qty), name="Produced", color=COL_BEFORE) %>%
  hc_add_series(td, "line", hcaes(x=week_ts, y=demand_qty),   name="Demand", color=COL_CAP, dashStyle="ShortDot") %>%
  hc_tooltip(shared=TRUE, crosshairs=TRUE) %>%
  hc_lang_safe(hc_lang)
```
...
compute_weekly <- function(x){
  ...
  prod %>% group_by(week) %>%
    summarise(
      # Throughput counted at PACK only (avoid double counting)
      produced_qty = sum(produced_qty[operation_step == "PACK"], na.rm=TRUE),

      # Reject & rework as flow-loss signals
      rejected_qty = sum(rejected_qty[operation_step %in% c("QC_INLINE","QC_FINAL")], na.rm=TRUE),
      rework_qty   = {
        if ("rework_qty" %in% names(prod)) {
          sum(rework_qty[operation_step == "REWORK"], na.rm=TRUE) +
            sum(produced_qty[operation_step == "REWORK" & (is.na(rework_qty) | rework_qty == 0)], na.rm=TRUE)
        } else {
          sum(produced_qty[operation_step == "REWORK"], na.rm=TRUE)
...
prod_wk <- B$prod %>%
  ...
  group_by(week) %>%
  summarise(
    # Throughput counted only at PACK (finished output)
    produced_units = sum(produced_qty[operation_step == "PACK"], na.rm=TRUE),

    # Reject signals mostly come from QC steps
    rejected_units = sum(rejected_qty[operation_step %in% c("QC_INLINE","QC_FINAL")], na.rm=TRUE),

    # Rework volume is what went through REWORK step (fallback if rework_qty is empty)
    rework_units   = {
      if ("rework_qty" %in% names(B$prod)) {
        sum(rework_qty[operation_step == "REWORK"], na.rm=TRUE) +
          sum(produced_qty[operation_step == "REWORK" & (is.na(rework_qty) | rework_qty == 0)], na.rm=TRUE)
