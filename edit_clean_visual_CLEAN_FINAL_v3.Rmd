...
highchart() %>%
  ...
  hc_subtitle(text = "Size = MOQ×Prod LT | Color = MOQ×Unit Price (cash tied) | Outline = Compliance | Label = CRITICAL (non-Yes & cash tied ≥ $3k)") %>%
  hc_chart(type = "bubble", zoomType = "xy") %>%
  hc_xAxis(
    title = list(text = "Production lead time (days)"),
    plotLines = list(
      list(value = 14, color = COL_GRAY_M, dashStyle = "Dash", width = 1,
           label = list(text = "14d", style = list(color = COL_GRAY_M)))
    )
  ) %>%
  hc_yAxis(title = list(text = "MOQ per order")) %>%
  hc_colorAxis(
    min = 0,
    max = cash_max,
    minColor = COL_BLUE_50,
    maxColor = clr_navy,
    labels = list(format = "${value:,.0f}"),
    title = list(text = "Cash tied (MOQ×Unit price, USD)")
  ) %>%
  hc_add_series(
    name = "Materials",
  ...
  hc_plotOptions(
    bubble = list(
      ...
      dataLabels = list(
        enabled = TRUE,
        allowOverlap = TRUE,
        formatter = JS("function(){ return this.point.is_critical ? 'CRITICAL' : ''; }"),
        style = list(fontWeight = "bold", color = clr_red, textOutline = "none")
      )
    )
  ) %>%
  hc_tooltip(
...
# Priority list (Top 20 by pressure), colored by compliance
p_top20 <- pressure_tbl %>% slice_head(n = 20) %>%
  mutate(
...
highchart() %>%
  hc_chart(type = "bar") %>%
  hc_title(text = "Top 20 pressure materials (priority list)") %>%
  hc_subtitle(text = "Bar color = Compliance | Pressure = MOQ×LT") %>%
  hc_xAxis(type = "category") %>%
  hc_yAxis(title = list(text = "Pressure (MOQ×LT)"), labels = list(format = "{value:,.0f}")) %>%
  hc_add_series(name = "Pressure", data = bar_56_points, showInLegend = FALSE) %>%
  hc_tooltip(
    useHTML = TRUE,
    formatter = JS(sprintf(
"function(){
  var p = this.point || {};
  var code = p.material_code || 'N/A';
  var origin = p.supplier_country || 'N/A';
  var supp = p.supplier_id || 'N/A';
  var comp = p.compliance_status || 'N/A';
  var crit = p.is_critical ? 'CRITICAL' : '';

  return '<b>' + code + '</b> (' + origin + ') ' + (crit ? '<span style=\"color:%s\">[' + crit + ']</span>' : '') + '<br/>' +
         'Supplier: <b>' + supp + '</b><br/>' +
         'Compliance: <b>' + comp + '</b><br/>' +
         'MOQ: <b>' + Highcharts.numberFormat(p.moq,0) + '</b> | LT: <b>' + Highcharts.numberFormat(p.lt,0) + ' days</b><br/>' +
         'Pressure (MOQ×LT): <b>' + Highcharts.numberFormat(p.y,0) + '</b><br/>' +
         'Unit price: <b>$' + (p.unit_price==null ? 'NA' : Highcharts.numberFormat(p.unit_price,2)) + '</b><br/>' +
         'Cash tied: <b>$' + (p.cash_tied==null ? 'NA' : Highcharts.numberFormat(p.cash_tied,0)) + '</b>';
}", clr_red))
  ) %>%
  hc_add_theme(hc_theme_vscc())
```
...
cts_max <- suppressWarnings(max(_df$cts, na.rm = TRUE))
...
highchart() %>%
  ...
  hc_xAxis(
    ...
    plotLines = list(
      list(value = otif_mid, color = COL_GRAY_M, dashStyle = "Dash", width = 2,
           label = list(text = paste0("Median ", round(otif_mid,1), "%"), style = list(color = COL_GRAY_M)))
    )
  ) %>%
  hc_yAxis(
    title = list(text = "GM%"),
    plotLines = list(
      list(value = gm_mid, color = COL_GRAY_M, dashStyle = "Dash", width = 2,
           label = list(text = paste0("Median ", round(gm_mid,2), "%"), style = list(color = COL_GRAY_M)))
    )
  ) %>%
  hc_colorAxis(
    min = 0,
