...
CIpur_lookup  <- read_excel("CIpurlookup1_1.xlsx")
...
norm_trade_unit <- function(x){
  ...
  dplyr::case_when(
    x1 %in% c("box", "outerbox") ~ "outer_box",
    x1 == "palletlayer"          ~ "pallet_layer",
    TRUE                         ~ x1
  )
}
#~~
...
supplier_master <- tribble(
  ...
  ~certified,
  "Mono Packaging Materials","Pack","France","At delivery",10,NA,100,19,450,31,21,TRUE,
  "Trio PET PLC","PET","Spain","At delivery",5,NA,100,20,500,8,5,TRUE,
  "Miami Oranges","Orange","United States","When ordering",10,"Truck",125,30,750,30,22,TRUE,
  "Miami Oranges","Orange","United States","When ordering",25,"Boat",125,20,500,30,22,TRUE,
  ...
  "Seitan Vitamins","Vitamin C","China","When ordering",20,NA,100,24,600,50,45,TRUE
)
supplier_mode_choices <- supplier_master %>%
  transmute(vendor = supplier, mode = str_to_lower(mode)) %>%
  filter(!is.na(mode), mode != "") %>%
  distinct() %>%
  group_by(vendor) %>%
  summarise(modes = list(unique(mode)), .groups = "drop")

supplier_mode_default <- supplier_mode_choices %>%
  mutate(mode = purrr::map_chr(modes, dplyr::first)) %>%
  select(vendor, mode)
...
fg_warehouse_week <- function(week, state, lookups, operations_constants,decisions_ops,
                              sales_exec_w = tibble(), produced_w = tibble()){
  ...
  scrap_cost_week <- state$fg_stock %>%
    ...
    mutate(
      ...
      basic_sales_price = replace_na(basic_sales_price,0),
      units_per_pallet = replace_na(units_per_pallet, NA_real_),
      obsolete_pallets = ifelse(is.na(units_per_pallet) | units_per_pallet<=0, 0, obsolete_units/units_per_pallet),
      scrap_cost = obsolete_units * basic_sales_price
    ) 
  scrap_cost_val <- sum(scrap_cost_week$scrap_cost, na.rm=TRUE)
  obsolete_pallets <- sum(scrap_cost_week$obsolete_pallets, na.rm=TRUE)
  
  # ---- OUTBOUND picking workload (depends on trade_unit)
  # Need: sales_exec_w has trade_unit, delivered_units, and df_sku units_per_box etc.
  ship_lines <- 0
  pick_pallet_equiv <- 0
  pick_boxes <- 0
  
  if (!is.null(sales_exec_w) && nrow(sales_exec_w) > 0){
    se <- sales_exec_w %>%
      left_join(df_sku %>% select(sku, units_per_pallet, units_per_layer, units_per_box), by="sku") %>%
      mutate(
        trade_unit = norm_trade_unit(trade_unit),
        delivered_units = replace_na(delivered_units, 0),
        
        # pallet-equivalent picks for pallet / pallet_layer
        u_pallet = replace_na(units_per_pallet, NA_real_),
        u_layer  = replace_na(units_per_layer, NA_real_),
        
        pallet_equiv_pick = case_when(
          delivered_units <= 0 ~ 0,
          trade_unit %in% c("pallet","pallet_layer") ~ pallet_equiv_shipped(
            delivered_units, trade_unit, u_pallet, u_layer, units_per_box = NA_real_
          ),
          TRUE ~ 0
        ),
        
        # box picks for outer_box
        u_box = replace_na(units_per_box, NA_real_),
        boxes_pick = case_when(
          delivered_units <= 0 ~ 0,
          trade_unit == "outer_box" & is.finite(u_box) & u_box > 0 ~ ceiling(delivered_units / u_box),
          trade_unit == "outer_box" ~ 0,
          TRUE ~ 0
        )
      )
    
    ship_lines <- sum(se$delivered_units > 0, na.rm=TRUE)
    pick_pallet_equiv <- sum(se$pallet_equiv_pick, na.rm=TRUE)
    pick_boxes <- sum(se$boxes_pick, na.rm=TRUE)
  }
  
  # ---- INBOUND store workload (pallets coming from production)
  store_pallets <- 0
  if (!is.null(produced_w) && nrow(produced_w) > 0){
    store_tbl <- produced_w %>%
      left_join(df_sku %>% select(sku, units_per_pallet), by="sku") %>%
      mutate(
        units_per_pallet = replace_na(units_per_pallet, NA_real_),
        inbound_units = replace_na(produced_units,0) + replace_na(outsourced_units,0),
        inbound_pallets = ifelse(is.na(units_per_pallet) | units_per_pallet<=0, 0, inbound_units/units_per_pallet)
      )
    store_pallets <- sum(store_tbl$inbound_pallets, na.rm=TRUE)
  }
  
  # ---- overflow moves + housekeeping + obsolete handling
  housekeeping_hours <- fg_const$housekeeping_hours_per_week %||% 0
  
  workload_minutes <-
    store_pallets      * (fg_const$store_pallet_minutes %||% 0) +
    ship_lines         * (fg_const$pick_orderline_minutes %||% 0) +
    pick_pallet_equiv  * (fg_const$pick_pallet_minutes %||% 0) +
    pick_boxes         * (fg_const$pick_outerbox_minutes %||% 0) +
    overflow_pallets   * (fg_const$move_overflow_minutes %||% 0) +
    obsolete_pallets   * (fg_const$obsolete_pallet_minutes %||% 0)
  
  workload_hours <- workload_minutes / 60 + housekeeping_hours
  
  # ---- labor cost (perm + flex), add new decision knob n_fte_fg_wh (default 1)
  n_fte_fg <- decisions_ops$n_fte_fg_wh %||% 1L
  perm_rate <- fg_const$fte_cost_per_year / (52 * fg_const$fte_hours_per_week)
  flex_rate <- fg_const$flex_cost_per_hour
  
  perm_cap_hours <- n_fte_fg * fg_const$fte_hours_per_week
  perm_hours <- n_fte_fg * fg_const$fte_hours_per_week
  flex_hours <- pmax(0, workload_hours - perm_cap_hours)
  
  fg_labor_cost_week <- perm_hours * perm_rate + flex_hours * flex_rate
  ...
  tibble(
    ...
    fg_store_pallets=store_pallets,
    fg_ship_orderlines=ship_lines,
    fg_pick_pallet_equiv=pick_pallet_equiv,
    fg_pick_boxes=pick_boxes,
    
    fg_workload_hours=workload_hours,
    fg_perm_hours=perm_hours,
    fg_flex_hours=flex_hours,
    fg_labor_cost=fg_labor_cost_week,
    
    scrap_cost=scrap_cost_val
  )
}
...
rm_warehouse_week <- function(week, state, lookups, operations_constants,
  ...
  df_comp <- lookups$df_component %>%
    mutate(
      ...
      component = dplyr::case_when(
        component_key == "pack_1_liter" ~ "pack", # map về "pack" để match rm_stock
        TRUE                            ~ component_key
      ),
      units_per_pallet = as.numeric(readr::parse_number(as.character(pallet_qty)))
    ) %>%
    select(component, units_per_pallet)
  #Update 0.2.1
  ...
  perm_hours <- n_fte_rm * rm_const$fte_hours_per_week ##18/12 luôn đủ giờ FTE
  flex_hours <- pmax(0, workload_hours - perm_cap_hours)
...
admin_cost_week <- function(week, orders_w, sales_exec_w, purchasing_constants, df_component){
  ...
  sup_master <- purchasing_constants$supplier_master %>%
    ...
    mutate(
      ...
      mode_key = ifelse(is.na(mode) | mode == "", "default", mode)
    )##22/12
  ...
  orders_ext <- orders_w %>%
    { if (!"mode" %in% names(.)) mutate(., mode = NA_character_) else . } %>% ##22/12
    mutate(
      ...
      mode = str_to_lower(mode),
      mode_key = ifelse(is.na(mode) | mode == "", "default", mode)##22/12
    ) %>%
    left_join(comp_pallet_tbl, by = "component_key") %>%
    left_join(sup_master, by = c("vendor", "mode_key")) %>% #22/12
    mutate(
      # trade size
      trade_size_units = case_when(
...
refresh_ci_sales <- function(sales_decision_cp, CIsale_lookup) {
  ...
  sales_decision_clean <- sales_decision_cp %>%
    mutate(
      customer             = norm_customer(customer),
      promotional_pressure = str_to_lower(promotional_pressure),
      trade_unit           = norm_trade_unit(trade_unit),   # <--- PATCH
      promotion_horizon    = str_to_lower(promotion_horizon),
      order_deadline       = str_replace_all(order_deadline, "\\s+",""),
      payment_term         = as.character(payment_term),
...
apply_purch_agreement_component <- function(supplier_params_default, CIpur_lookup, idx_by_component){
  ...
  base_vendor_tbl <- supplier_params_default %>%
    select(component, base_vendor = vendor, base_mode = mode)#22/12
  ...
  out_list <- lapply(names(idx_by_component), function(comp){
    ...
    base_mode <- base_vendor_tbl$base_mode[match(comp, base_vendor_tbl$component)]##22/12
...
supplier_params_default <- tibble(
  ...
  trade_unit = "pallet",
  payment_term = "4"
) %>% left_join(supplier_mode_default, by = c("vendor")) %>% ##22/12
  mutate(
    quality = str_to_lower(quality),
    delivery_window = str_to_lower(delivery_window),
    trade_unit = str_to_lower(trade_unit),
...
rm_replenishment_week <- function(week, state, plan_w, decisions_sc,
  ...
  rm_need <- plan_w %>%
    ...
    mutate(
      # DÙNG demand_units (weekly) chứ không phải planned_units
      rm_week_need = demand_units * qty_per_unit
    ) %>%
    group_by(component) %>%
    summarise(rm_week_need = sum(rm_week_need, na.rm = TRUE), .groups = "drop")##18/12 
  ...
  reorder_tbl <- rm_need %>%
    ...
    mutate(
      ...
      order_units_raw = pmax(0, (lot_units + target_ss_units) - net_stock_units)
    ) %>%
    # join supplier params for trade_unit + vendor + ci
    left_join(supplier_params %>% 
                select(component, ci_purch, vendor, trade_unit, mode),##22/12
              by="component") %>%
    mutate(
      trade_unit = str_to_lower(replace_na(trade_unit, "pallet"))
    )
  ...
  reorder_tbl <- reorder_tbl %>%
    ...
    mutate(
      ...
      trade_size_units = replace_na(trade_size_units, 1),
      order_units = ifelse(
  ...
  reorder_tbl <- reorder_tbl %>%
    ...
    left_join(sup_master %>% select(vendor, mode_key, lead_time_days),
              by = c("vendor", "mode_key")) %>%##22/12
    mutate(
  ...
  lead_tbl <- reorder_tbl %>%
    group_by(component, vendor,, mode_key) %>%
  ...
  receipts_w <- orders %>%
    ...
    left_join(lead_tbl, by=c("component","vendor", "mode_key")) %>%#22/12
    transmute(
      week_order = week,
      component,
      vendor,
...
fulfill_demand_week <- function(week, state, demand_w, produced_w, df_sku,
  ...
  avail <- fg_stock %>%
    full_join(
      produced_w %>%
        transmute(sku, produced_total = replace_na(produced_units,0) + replace_na(outsourced_units,0)),
      by="sku"
    ) %>%
    mutate(
      fg_units = replace_na(fg_units, 0),
      produced_total = replace_na(produced_total, 0),
      available_units = fg_units + produced_total
    )
  ##--
  ...
  sales_exec <- demand_alloc %>%
    ...
    left_join(
      df_sku %>%
        select(sku, basic_sales_price, units_per_pallet, units_per_layer, units_per_box),
      by = "sku"
    ) %>%
    mutate(
      trade_unit = norm_trade_unit(trade_unit),
      
      u_pallet = units_per_pallet,
      u_layer  = units_per_layer,
      u_box    = units_per_box,
      
      pallets_shipped = purrr::pmap_dbl(
        list(delivered_units, trade_unit, u_pallet, u_layer, u_box),
        pallet_equiv_shipped
      ),
    ...
    select(week, customer, sku, demand_units, delivered_units, attained_ci,trade_unit,   # <--- KEEP trade_unit
...
update_state_week <- function(state, receipts_w, rm_used_w, produced_w, sales_exec, df_sku){
  ...
  rm_stock <- rm_stock %>%##19/12
    mutate(
      component = norm_component(component),
      component = dplyr::case_when(
        component == "pack_1_liter" ~ "pack",
        TRUE ~ component
      )
    ) %>%
    group_by(component) %>%
  ...
  fg_stock <- state$fg_stock %>%
    full_join(##balance patch
      produced_w %>%
        group_by(sku) %>%
        summarise(produced = sum(replace_na(produced_units,0) + replace_na(outsourced_units,0)), .groups="drop"),
      by="sku"
    ) %>%
...
finance_aggregate_round <- function(flows, finance_constants, operations_constants){ #fix patch beta 0.0.1
  ...
  initial_inv_cost <- finance_constants$initial_inventory_cost %||% 0
  purchase_costs <- sum(flows$purchasing$purchase_cost_total, na.rm=TRUE) +initial_inv_cost
  ...
  warehouse_costs <- sum(flows$warehousing$fg_location_cost, na.rm=TRUE) + ##update beta 0.0.7
    ...
    sum(flows$warehousing$fg_labor_cost, na.rm=TRUE) +   #16/12
  ...
  if (!is.null(flows$sales) && nrow(flows$sales) > 0) {
    dist_costs <- flows$sales %>%
      mutate(dist_cost = purrr::map_dbl(
        pallets_shipped,
  ...
  investment_total <- fixed_investment + stock_investment
...
engine_round <- function(state0, decisions, constants, lookups, exogenous, n_weeks=26){
  ...
  for (w in 1:n_weeks){
    ...
    rm_receipts_w <- state$rm_pipeline %>%
      filter(eta_week == w) %>%
      left_join(
        decisions$purchasing$supplier_params %>% select(component, delivery_reliability),
        by = "component"
      ) %>% #update beta 0.1.1
      mutate(
        delivery_reliability = replace_na(delivery_reliability, 100),
        delivery_reliability = ifelse(delivery_reliability <= 1, delivery_reliability * 100, delivery_reliability),
        delivery_reliability = pmin(pmax(delivery_reliability, 0), 100)
      ) %>%
      transmute(
        week = w,
        component,
        received_qty_units = order_qty_units * delivery_reliability / 100
      ) %>%  ##update beta 0.1.1
    ...
    flows$purchasing_receipts <- bind_rows(flows$purchasing_receipts, rm_receipts_w)#update 19/12
    ...
    flows$warehousing <- bind_rows(
      flows$warehousing,
      fg_warehouse_week(week = w,
                        state = state,
                        lookups = lookups,
                        operations_constants = constants$operations,
                        decisions_ops = decisions$operations,
                        sales_exec_w = sales_exec,               
                        produced_w   = prod_rep$production        )) ##Update 16/12
...
supplier_params_round0 <- tibble::tibble(
  ...
  left_join(supplier_mode_default, by = c("vendor")) %>%
  dplyr::mutate(
    component = norm_component(component),
    vendor = as.character(vendor),
...
ops_round0 <- list(
  rm_pallet_locations = 1287L,
  fg_pallet_locations = 705L,
  n_fte_fg_wh = 2L,
  n_fte_rm_wh = 14L,
  num_shifts  = 5L,
...
# valuation of initial RM stock to avoid optimistic P&L when no purchases are needed early
initial_inv_cost <- rm_init_tbl %>%
  dplyr::left_join(df_component %>% dplyr::select(component, basic_price), by = "component") %>%
  dplyr::left_join(supplier_params_round0 %>% dplyr::select(component, ci_purch), by = "component") %>%
  dplyr::mutate(
    basic_price = dplyr::coalesce(basic_price, 0),
    ci_purch = dplyr::coalesce(ci_purch, 1),
    inv_value = init_units * basic_price * ci_purch
  ) %>%
  dplyr::summarise(initial_inventory_cost = sum(inv_value, na.rm = TRUE), .groups = "drop") %>%
  dplyr::pull(initial_inventory_cost)

finance_constants$initial_inventory_cost <- initial_inv_cost
constants$finance <- finance_constants
...
# =========================
# QUICK SANITY (for Optuna)
# =========================
quick_sanity_metrics <- function(out, state0, constants){
  
  flows <- out$flows
  fr    <- out$finance_round
  
  # FG mass balance
  fg_open <- sum(state0$fg_stock$units, na.rm=TRUE)
  fg_prod <- sum(flows$production$produced_units, na.rm=TRUE) + sum(flows$production$outsourced_units, na.rm=TRUE)
  fg_ship <- sum(flows$sales$delivered_units, na.rm=TRUE)
  fg_close<- sum(out$state_end$fg_stock$units, na.rm=TRUE)
  fg_obs  <- sum(out$state_end$fg_stock$obsolete_units, na.rm=TRUE)
  fg_balance <- fg_open + fg_prod - fg_ship - fg_close - fg_obs
  
  # pallets sanity
  pallets_bad <- flows$sales %>%
    dplyr::summarise(n_bad = sum(!is.finite(pallets_shipped) | pallets_shipped < 0, na.rm=TRUE)) %>%
    dplyr::pull(n_bad)
  
  # distribution compare (weekwise vs linewise)
  dist_weekwise <- 0
  if (nrow(flows$sales) > 0){
    pallets_by_week <- flows$sales %>%
      dplyr::group_by(week) %>%
      dplyr::summarise(pallets_week = sum(pallets_shipped, na.rm=TRUE), .groups="drop")
    
    dist_weekwise <- sum(purrr::map_dbl(
      pallets_by_week$pallets_week,
      ~calc_distribution_cost(.x, finance_constants = constants$finance)
    ), na.rm=TRUE)
  }
  dist_linewise <- fr$distribution_costs
  dist_ratio <- ifelse(dist_linewise > 0, dist_weekwise / dist_linewise, NA_real_)
  
  list(
    fg_balance = fg_balance,
    pallets_bad = pallets_bad,
    dist_weekwise = dist_weekwise,
    dist_linewise = dist_linewise,
    dist_ratio = dist_ratio,
    finance_bad = (!is.finite(fr$ROI_pred) || !is.finite(fr$operating_profit) || !is.finite(fr$cogs))
  )
}


quick_sanity_ok <- function(out, state0, constants,
                            eps_balance = 1e-6,
                            max_dup_receipts = 0L,
                            dist_ratio_min = 0,      # weekwise should NOT exceed linewise
                            dist_ratio_max = 1.05,   # allow tiny numerical wiggle
                            max_pallets_bad = 0L){
  m <- quick_sanity_metrics(out, state0, constants)
  
  if (!is.finite(m$fg_balance) || abs(m$fg_balance) > eps_balance) return(FALSE)
  if (!is.finite(m$pallets_bad) || m$pallets_bad > max_pallets_bad) return(FALSE)
  if (!is.finite(m$dup_receipts) || m$dup_receipts > max_dup_receipts) return(FALSE)
  if (isTRUE(m$finance_bad)) return(FALSE)
  
  # Distribution double-count guard:
  # - linewise should be >= weekwise (because base cost gets counted many times linewise)
  # - so ratio should be <= ~1
  if (is.finite(m$dist_weekwise) && is.finite(m$dist_linewise) && m$dist_linewise > 0) {
    rel_gap <- abs(m$dist_weekwise - m$dist_linewise) / m$dist_linewise
    if (!is.finite(rel_gap) || rel_gap > 0.08) return(FALSE)  # allow 8% gap
  }
  
  TRUE
}
##--
validate_distribution_tiers <- function(fin){
  tiers <- fin$distribution$provider_tiers %>% dplyr::arrange(pallet_min)
  
  if (!any(is.infinite(tiers$pallet_max))) {
    stop("Distribution tiers: pallet_max must include Inf.", call.=FALSE)
  }
  if (any(diff(tiers$rate_per_pallet) > 1e-12)) {
    stop("Distribution tiers: rate_per_pallet must be non-increasing with volume.", call.=FALSE)
  }
  invisible(TRUE)
}
# gọi 1 lần trước optimize
validate_distribution_tiers(constants$finance)
#--
...
objective_r <- function(trial){
  ...
  supplier_params_trial <- supplier_params_trial %>%
    rowwise() %>%
    mutate(
      mode = {
        mode_list <- supplier_mode_choices$modes[
          match(vendor, supplier_mode_choices$vendor)
        ][[1]]
        if (is.null(mode_list) || length(mode_list) == 0) {
          NA_character_
        } else if (length(mode_list) == 1) {
          mode_list[[1]]
        } else {
          suggest_cat_r(trial, paste0("mode_", safe_key(component)), mode_list)
        }
      }
    ) %>%
    ungroup()
  ...
  n_fte_rm_wh         <- suggest_int_r(trial, "n_fte_rm_wh", 3L, 20L)
  ...
  m <- quick_sanity_metrics(out_trial, state0_round1, constants)
  
  # fail-fast (cực rẻ)
  if (!is.finite(m$fg_balance) || abs(m$fg_balance) > 1e-3) return(-1e9)
  if (m$pallets_bad > 0) return(-1e9)
  if (!isFALSE(m$finance_bad)) return(-1e9)
...
reconstruct_best_decisions <- function(
  ...
  supplier_params_best <- supplier_params_best %>%
    rowwise() %>%
    mutate(
      mode = {
        mode_list <- supplier_mode_choices$modes[
        ...
        if (is.null(mode_list) || length(mode_list) == 0) {
          ...
          key <- paste0("mode_", safe_key(component))
          as.character(best_params[[key]] %||% mode_list[[1]])
        }
      }
    ) %>%
    ungroup()
...
m <- quick_sanity_metrics(out_best, state0_round1, constants)
print(m)
