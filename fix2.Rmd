...
## 0) LOAD DATA, CONSTANTS, ENGINE

```{r engine_code, echo=TRUE}
# =========================
# 0) LOAD DATA
# =========================
...
CIpur_lookup  <- read_excel("CIpurlookup1_2.xlsx")
...
sales_constants <- list(
  ...
  market_forces=list( volume_factor_min=0.90,
...
# FG value (prefer cost-based value when available)
# FG value (prefer cost-based value when available)
cost_cols <- c("basic_cost", "unit_cost", "cost", "cogs", "standard_cost",
               "standard_costs", "basic_cogs")

cost_col <- intersect(cost_cols, names(df_sku))
cost_col <- if (length(cost_col) > 0) cost_col[1] else character(0)

  ##29/12
  fg_val_tbl <- state$fg_stock %>%
    left_join(
  df_sku %>% select(sku, basic_sales_price, dplyr::any_of(cost_col)),
  by = "sku"
) %>%
    mutate(
  basic_sales_price = replace_na(basic_sales_price, 0),
  unit_cost = if (length(cost_col) > 0) .data[[cost_col]] else basic_sales_price,
  unit_cost = tidyr::replace_na(unit_cost, 0),
  fg_value  = units * unit_cost
)
...
fg_warehouse_week <- function(week, state, lookups, operations_constants,decisions_ops,
  ...
  ##29/12
  cost_cols <- c("basic_cost", "unit_cost", "cost", "cogs",
               "standard_cost", "standard_costs", "basic_cogs")
...
cost_col <- if (length(cost_col) > 0) cost_col[1] else character(0)
  ...
  scrap_cost_week <- state$fg_stock %>%
    left_join(
  df_sku %>% select(sku, basic_sales_price, units_per_pallet, dplyr::any_of(cost_col)),
  by = "sku"
) %>%
##
    mutate(
      ...
      scrap_cost = obsolete_units * unit_cost
    ) 
...
auto_assortment_phase3 <- function(
  ...
  if (is.null(sales_exec_hist) || is.null(df_sku)) {
    out <- base %>%
      transmute(customer, sku, active = base_active, slowmover = FALSE)
    ...
    if (!is.null(assortment_prior) && nrow(assortment_prior)>0){
      out <- out %>%
        ...
        select(customer, sku, active, slowmover)
    }
    return(out)
  }
  ...
  out <- base %>%
    ...
    transmute(customer, sku, active = active_auto, slowmover = slowmover)
...
sales_demand_week <- function(week, state, sales_decision_cp, sales_constants, exo,
                              assortment_cp=NULL, slowmover_cp=NULL){
  ...
  vfm_multiplier <- sales_constants$market_forces$value_for_money_multiplier %||% 1
slowmover_multiplier <- sales_constants$market_forces$slowmover_multiplier %||% 1

customer_tbl <- sales_constants$customer_master %||%
  tibble(customer = unique(bench$customer), value_for_money = FALSE)

customer_tbl <- customer_tbl %>%
  mutate(customer = norm_customer(customer)) %>%
  select(customer, value_for_money)

slow_tbl <- slowmover_cp
if (is.null(slow_tbl) && !is.null(assortment_cp) && "slowmover" %in% names(assortment_cp)) {
  slow_tbl <- assortment_cp %>% select(customer, sku, slowmover)
}
if (is.null(slow_tbl)) {
  slow_tbl <- tibble(customer = character(), sku = character(), slowmover = logical())
}
...
##29/12
  dem <- bench %>%
    ...
    left_join(customer_tbl, by = "customer") %>%
  left_join(slow_tbl, by = c("customer", "sku")) %>%##29/12
    mutate(
      volume_factor = replace_na(volume_factor, 1),               # <<<<<< FIX
      demand_week_pieces = as.numeric(demand_week_pieces),        # &lt;&lt;&lt;&lt;&lt;&lt; FIX an toàn
      value_for_money = replace_na(value_for_money, FALSE),
      slowmover = replace_na(slowmover, FALSE)         # <<<<<< FIX an toàn
    ) %>%
    ...
    mutate(
      ...
      promo_uplift = map_dbl(promotional_pressure, get_promo_uplift),
       promo_uplift = promo_uplift *
        ifelse(value_for_money, vfm_multiplier, 1) *
        ifelse(slowmover, slowmover_multiplier, 1),
...
rm_replenishment_week <- function(week, state, plan_w, decisions_sc,
  ...
  rm_need <- plan_w %>%
    ...
    mutate(
      ...
      rm_week_need = planned_units * qty_per_unit
    ) %>%
    group_by(component) %>%
...
finance_aggregate_round <- function(flows, finance_constants, operations_constants){ #fix patch beta 0.0.1
  ...
  if (!is.null(flows$sales) && nrow(flows$sales) > 0) {
    dist_costs <- flows$sales %>%
      ...
      group_by(week) %>%
summarise(
  pallets_shipped = sum(pallets_shipped, na.rm = TRUE),
  .groups = "drop"
) %>%
      ##
...
engine_round <- function(state0, decisions, constants, lookups, exogenous, n_weeks=26){
  ...
  for (w in 1:n_weeks){
    ...
    demand_w <- sales_demand_week(
      ...
      slowmover_cp = decisions$sales$slowmover_cp
    )
...
rm_lot0 <- setNames(c(2L,1.0L,1.3L,2.5L,4L), c("pack","pet","orange","mango","vitamin_c"))

fg_ss0  <- setNames(c(1.4,2.0,1.5,1.3,1.7,1.2), unique(df_sku$sku))
fg_int0 <- setNames(c(9L,14,12,8,9,9), unique(df_sku$sku))  # 10 days all SKUs
frozen0 <- 3L
...
assortment_cp_round0 <- tibble::tribble(
  ~customer, ~sku, ~active,
  "Food & Groceries", "Fressie Orange 1 liter", TRUE,
  "Food & Groceries", "Fressie Orange/C-power 1 liter", TRUE,
  "Food & Groceries", "Fressie Orange/Mango 1 liter", TRUE,
  "Food & Groceries", "Fressie Orange PET", TRUE,
  "Food & Groceries", "Fressie Orange/C-power PET", TRUE,
  "Food & Groceries", "Fressie Orange/Mango PET", TRUE,
  "LAND Market", "Fressie Orange 1 liter", TRUE,
  "LAND Market", "Fressie Orange/C-power 1 liter", TRUE,
  "LAND Market", "Fressie Orange/Mango 1 liter", TRUE,
  "LAND Market", "Fressie Orange PET", TRUE,
  "LAND Market", "Fressie Orange/C-power PET", FALSE,
  "LAND Market", "Fressie Orange/Mango PET", TRUE,
  "Dominick's", "Fressie Orange 1 liter", FALSE,
  "Dominick's", "Fressie Orange/C-power 1 liter", FALSE,
  "Dominick's", "Fressie Orange/Mango 1 liter", FALSE,
  "Dominick's", "Fressie Orange PET", TRUE,
  "Dominick's", "Fressie Orange/C-power PET", TRUE,
  "Dominick's", "Fressie Orange/Mango PET", TRUE
) %>%
  dplyr::mutate(
    customer = norm_customer(customer),
    sku = stringr::str_squish(sku)
  )
slowmover_cp_round0 <- auto_assortment_phase3(
  benchmark_demand   = benchmark_demand,
  sales_exec_hist    = NULL,
  df_sku             = df_sku,
  sales_decision_cp  = sales_decision_cp_round0,
  assortment_prior   = assortment_cp_round0
) %>%
  dplyr::select(customer, sku, slowmover)
...
decisions_round0 <- list(
  sales = list(
    ...
    slowmover_cp = slowmover_cp_round0,
...
demand_w1 <- sales_demand_week(
  ...
  slowmover_cp = decisions_round0$sales$slowmover_cp
)
...
## 1) Xem nhanh kết quả ROI & P&L

```{r show-finance, echo=TRUE}
out_base$finance_round$ROI_pred
out_base$finance_round
```


```{r load-game-data, echo=TRUE}
# =========================
# 0B) LOAD GAME REFERENCE (TFC download)
# =========================
# =========================
# 0B) LOAD GAME REFERENCE (TFC download)
# =========================
...
game_files <- list(
  tfc_detail  = "TFC_0_1.xlsx",       # file component/supplier... chi tiết
  finance_kpi = "FinanceReport.xlsx"  # file P&L có ROI, GM...
)

# ---- 1) Đọc sheet Output trong FinanceReport (ROI, GM, cost lines)
finance_output <- readxl::read_excel(game_files$finance_kpi,
                                     sheet = "Output") %>%
  janitor::clean_names()

# ---- 2) Đọc TẤT CẢ sheet của TFC_0_1.xlsx, đặt tên list = clean_names(sheet)
tfc_sheets <- readxl::excel_sheets(game_files$tfc_detail)

tfc_detail <- tfc_sheets %>%
  set_names(janitor::make_clean_names(tfc_sheets)) %>%   # tên list: component, supplier, ...
  purrr::map(~ readxl::read_excel(game_files$tfc_detail, sheet = .x) %>%
               janitor::clean_names())

# ---- 3) Gom vào 1 list để dùng sau
game_ref <- list(
  tfc_detail     = tfc_detail,
  finance_output = finance_output
)
```
```r
```{r table compare}
# Engine: tổng purchase theo component (26 tuần)
engine_purchase_comp <- out_base$flows$purchasing %>%
  mutate(component = norm_component(component)) %>%
  group_by(component) %>%
  summarise(purchase_value_engine = sum(purchase_cost_total, na.rm = TRUE),
            .groups = "drop")

game_purchase_comp <- game_ref$tfc_detail$component %>%
  mutate(component_raw = .[[1]]) %>%
  transmute(
    component = norm_component(component_raw),
    purchase_value_previous_round = purchase_value_previous_round
  )

compare_purchase <- game_purchase_comp %>%
  left_join(engine_purchase_comp, by = "component") %>%
  mutate(diff = purchase_value_engine - purchase_value_previous_round)

compare_purchase


engine_admin_summary <- out_base$flows$distribution %>%
  summarise(
    inbound_orders      = sum(inbound_orders, na.rm = TRUE),
    inbound_orderlines  = sum(inbound_orderlines, na.rm = TRUE),
    inbound_admin_cost  = sum(admin_cost, na.rm = TRUE),
    inbound_transport   = sum(transport_cost, na.rm = TRUE)
  )

engine_admin_summary
